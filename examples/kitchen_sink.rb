...
# Password input with warning validation (soft failure - user can confirm)
api_key = Clack.password(
  message: "Enter your API key:",
  mask: "*",
  validate: lambda { |v|
    return "API key is required" if v.empty?

    Clack.warning("Short API key - are you sure?") if v.length < 10
  }
)
